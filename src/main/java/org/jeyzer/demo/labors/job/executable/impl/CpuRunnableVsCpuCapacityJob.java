...
public class CpuRunnableVsCpuCapacityJob extends ExecutableJob {
	...
	public CpuRunnableVsCpuCapacityJob(ExecutableJobDefinition def) {
		super(def);
	}
	...
	private void createCpuConsumingThreads() throws InterruptedException {
		...
		// Take max capacity by instantiating more threads than required
		int cpuMax = (int)(osmxBean.getAvailableProcessors() * 1.5);

		for (int i=0; i<cpuMax; i++) {
			Thread t = factory.newThread(new SubCpuRunnableVsCpuCapacityJob());
			cpuThreads.add(t);
			t.start();
		}
	...
	public final class SubCpuRunnableVsCpuCapacityJob implements Runnable{
