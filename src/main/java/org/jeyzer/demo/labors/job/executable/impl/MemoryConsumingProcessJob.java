...
public class MemoryConsumingProcessJob extends ExecutableJob {
	
	private List<Long> elements = new ArrayList<>();

	public MemoryConsumingProcessJob(ExecutableJobDefinition def) {
		super(def);
	}
	...
	private void consumeMemory() throws InterruptedException {
		for (long i=1; i<6000000; i++){ //around 600 Mb
			this.elements.add(i);
		}
