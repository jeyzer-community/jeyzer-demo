...
public class SystemCpuOverloadJob extends ExecutableJob {
	...
	public SystemCpuOverloadJob(ExecutableJobDefinition def) {
		super(def);
	}
	...
	private void createCpuConsumingThreads() throws InterruptedException {
		...
		// Try to take 25% of system CPU
		int cpuMax = (int)(osmxBean.getAvailableProcessors() * 0.5);

		for (int i=0; i<cpuMax; i++) {
			Thread t = factory.newThread(new SubSystemCpuOverloadJob());
			cpuThreads.add(t);
			t.start();
		}
	...
	public final class SubSystemCpuOverloadJob implements Runnable{
